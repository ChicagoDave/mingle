...
- **Stack**: TypeScript end to end — React Router 7 (framework mode, SSR), React, PostgreSQL with Drizzle ORM, Node 22 LTS. Background jobs via a Postgres-backed queue (pg-boss) rather than Redis, preserving a one-container-plus-Postgres install. Intended UI libraries: dnd-kit (card wall drag-drop), TipTap (rich text, replacing CKEditor).
- **UX preservation** means interaction design, layout, and CSS harvested from `mingle/app/views` and `mingle/public` — not the 2010-era Prototype.js JavaScript, which is not portable.
